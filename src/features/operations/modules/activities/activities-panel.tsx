...
export function ActivitiesPanel() {
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        description="Organiza llamadas, WhatsApp, visitas, citas y próximos pasos de la operación comercial."
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="Próximas" value={upcoming} />
      ...
      {session.role === "Gerente" ? (
        <Card className="border-yellow-500/20 bg-yellow-500/8 p-5">
          <div className="text-sm font-semibold text-slate-900">Riesgo de seguimiento</div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {overdueBySeller.length ? overdueBySeller.map((item) => (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3" key={item.seller}>
                <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{item.seller}</div>
                <div className="mt-1 text-base font-semibold text-slate-900">{item.value} vencidas</div>
              </div>
            )) : (
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        <Card className="overflow-hidden">
          ...
          {filteredActivities.length ? filteredActivities.map((activity) => (
            <button className={cn("grid w-full gap-4 border-b border-slate-100 px-6 py-5 text-left transition last:border-b-0 lg:grid-cols-[1.2fr_1fr_1fr_1fr_1fr] lg:items-center", selectedActivity?.id === activity.id ? "bg-red-50" : "hover:bg-slate-100")} key={activity.id} onClick={() => setSelectedActivityId(activity.id)} type="button">
              <div><div className="font-semibold text-slate-900">{activity.titulo}</div><div className="mt-1 text-xs text-slate-500">{activity.tipo} / {activity.prioridad}</div></div>
              <div className="text-sm text-slate-500">{getSubject(activity, leads, customers, files)}</div>
              <div className="text-sm text-slate-500">{formatDate(activity.fechaProgramada)}</div>
              ...
              <div className="text-sm text-slate-500">{activity.vendedorNombre}</div>
            </button>
          )) : <div className="p-8 text-center text-sm text-slate-500">Aún no hay actividades para este alcance. Registrá una próxima acción desde un lead, cliente o expediente.</div>}
        </Card>
...
function ActivityDetail({ activity, canManage, onCancel, onComplete, subject }: { activity: ActivityRecord | null; canManage: boolean; onCancel: (id: string) => void; onComplete: (id: string) => void; subject: string }) {
  if (!activity) return <Card className="p-8 text-center"><ClipboardList className="mx-auto h-10 w-10 text-slate-400" /><h3 className="mt-4 text-lg font-semibold text-slate-900">Sin seleccion</h3><p className="mt-2 text-sm text-slate-500">Selecciona una actividad para revisar el seguimiento.</p></Card>;
  return <Card className="p-6"><div className="flex items-start justify-between gap-4"><div><Badge tone={statusTone(activity.estado)}>{activity.estado}</Badge><h3 className="mt-4 text-xl font-semibold text-slate-900">{activity.titulo}</h3><p className="mt-1 text-sm text-slate-500">{activity.tipo} / {activity.prioridad}</p></div><CalendarClock className="h-6 w-6 text-red-600" /></div><div className="mt-6 space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-5"><Detail label="Contacto" value={subject} /><Detail label="Sucursal" value={activity.sucursalNombre} /><Detail label="Responsable" value={activity.vendedorNombre} /><Detail label="Programada" value={formatDate(activity.fechaProgramada)} /><Detail label="Creada" value={formatDate(activity.fechaCreacion)} /></div>{activity.descripcion ? <p className="mt-5 text-sm leading-6 text-slate-600">{activity.descripcion}</p> : null}{activity.resultado ? <p className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">Resultado: {activity.resultado}</p> : null}{canManage && activity.estado === "Pendiente" ? <div className="mt-6 grid gap-3"><Button onClick={() => onComplete(activity.id)} variant="success"><CheckCircle2 className="h-4 w-4" />Marcar completada</Button><Button onClick={() => onCancel(activity.id)} variant="danger"><XCircle className="h-4 w-4" />Cancelar actividad</Button></div> : null}</Card>;
}

function SessionRequired() { return <Card className="p-8 text-center"><Users className="mx-auto h-10 w-10 text-slate-400" /><h2 className="mt-4 text-xl font-semibold text-white">Inicia sesión para continuar</h2><p className="mt-2 text-sm text-slate-500">Inicia sesión para consultar actividades comerciales.</p><Link className="mt-5 inline-flex h-11 items-center justify-center rounded-lg bg-blue-600 px-5 text-sm font-semibold text-white" href="/panel">Ir a inicio de sesión</Link></Card>; }
function Metric({ label, value }: { label: string; value: number }) { return <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 text-lg font-semibold text-slate-900">{value}</div></div>; }
function Filter({ children, onChange, value }: { children: React.ReactNode; onChange: (value: string) => void; value: string }) { return <select className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-900 outline-none focus:border-blue-500" onChange={(event) => onChange(event.target.value)} value={value}>{children}</select>; }
function Detail({ label, value }: { label: string; value: string }) { return <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4 last:border-b-0 last:pb-0"><span className="text-sm text-slate-500">{label}</span><span className="max-w-[220px] text-right text-sm font-semibold text-slate-900">{value}</span></div>; }
