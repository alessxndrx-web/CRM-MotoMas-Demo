...
| **PL-4** | ~~**Sin turno.**~~ **Resuelto en CB4-D3.** Una venta de mostrador que cobra **efectivo** exige un `PosCashShift` abierto del operador y la sucursal de la sesión, y guarda cuál en `PosSale.shiftId`. Tarjeta y transferencia se cobran sin turno y dejan `shiftId` nulo: no tocan el cajón. **Sigue sin pertenecer a un `CashSession`**, que es el turno de la línea de motocicletas y otra cosa; lo que se resolvió es la atribución del efectivo del mostrador a su propio turno. La regla vive dentro de la transacción del cobro, con `SELECT … FOR UPDATE` sobre el turno. |
